...
export class DisplaySubscriptionsAgendaDetailsComponent extends ParentComponent
  implements OnInit {
  ...
  @Output() inschrijvingenList = new EventEmitter<any>();


  ngOnInit() {

    let subInschrijvingen: Observable<any>
    if (this.AgendaId) {
      subInschrijvingen = this.inschrijvingService.getSubscriptionsEvent$(+this.AgendaId)
        .pipe(
          catchError(err => of(new Array<InschrijvingItem>()))
        );
    } else {
      subInschrijvingen = this.inschrijvingService.getAll$()
        .pipe(
          catchError(err => of(new Array<InschrijvingItem>()))
        );
    }
      ...
      forkJoin([subInschrijvingen, subLeden, subAgenda])
        .subscribe({
          next: (data) => {
            ...
            this.inschrijvingenList.emit(this.agendaWithInschrijvingen);
          },
